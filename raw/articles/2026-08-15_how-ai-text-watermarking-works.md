...
compiled_to: "[[src_how-ai-text-watermarking-works]]"
source: declaude.org
---
...
When a model is mid-sentence, it doesn't know "the next word." It has a shortlist, like autocomplete, with preferences. Each roll sweeps the shortlist, lands on one word (odds matching the bars) and drops it into the sentence. Every landing makes a perfectly good sentence.
...
Two things make this sneaky. The nudge is mild: a red word can still win — it's just a little less likely. And the colouring is not a fixed property of the word: the key computes it from a short run of the words just before, so the same candidate is green after one prefix and red after another. Only the overall lean toward green accumulates, and only the key-holder knows which words were green where.
...
Detection doesn't read the text or judge its style. The detector replays the key-holder's colouring over the words and counts how many came up green. Without a mark (or without the right key), green should win about half the time. A coin flip. With the wrong key the split is meaningless, and the count sits at chance.
...
Each word's colouring is derived from a short run of the words just before it (one to a handful, depending on the scheme). So a position only counts as evidence if a short window of the original wording (the word plus its neighbours) survives intact. A run that matches the original exactly can be counted; faded/new wording has only coin-flip noise.

The verdict reads the surviving fraction measured from the highlights, projected to a 1,500-word document. Two things to notice: how much a "heavy edit" leaves standing, and how far toward a full rewrite you have to drag before the evidence actually dies.
...
- Anthropic, *How Claude marks AI-generated content* (Help Center, Aug 2026)
- declaude's own known-key experiments: re-composition collapses KGW/EXP detection to chance (AUC 0.99 → ≈0.5), context-free unigram marks survive (0.73–0.84); outline-level regeneration is the only answer we know for meaning-space marks.
