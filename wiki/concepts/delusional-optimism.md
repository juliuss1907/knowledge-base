---
type: concept
status: draft
main_tag: productivity
sub_tags: [psychology, opinion]
topic: personal-excellence-mindset
sources:
  - "[[src_ill-make-you-believe-you-can-achieve]]"
last_updated: 2026-07-21
---

# Delusional Optimism

## Definition

Delusional optimism (lạc quan ảo tưởng) là trạng thái giữ khái niệm bản thân ở mức cao hơn những gì khách quan đúng. Là tin rằng mọi thứ sẽ diễn ra theo cách có lợi cho mình, ngay cả khi chưa có bằng chứng vật lý trước đó. Đây là "cheat code" của những người thành công.

## Key ideas

- Kết hợp với Law of Assumption của Neville Goddard
- Tin bạn là vĩ đại hơn bạn đang là, ngay cả khi chưa có kết quả để chứng minh
- Cần cân bằng với khiêm tốn - vẫn có rất nhiều điều cần học
- Ví dụ: Muhammad Ali, Michael Jordan, Conor McGregor, Kobe Bryant đều có mức độ tự tin này
- Khác với ảo tưởng không có cơ sở - cần kết hợp với hành động nhất quán

## Related concepts

- [[law-of-assumption]]
- [[internal-locus-of-control]]

## Sources

- [[src_ill-make-you-believe-you-can-achieve]]

## Notes

