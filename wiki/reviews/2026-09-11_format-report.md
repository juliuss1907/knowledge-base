# Format Validation — 2026-09-11

**Status:** APPLIED — Applied 2026-09-14 by Connor
**Issues found:** 399
**Created:** 2026-09-11 23:15:53
**Validator:** format-validator
**Files checked:** 1042 (573 concepts + 197 sources + 34 indexes + 238 topics)
**ERRORs**: 0
**WARNINGS**: 399
**INFOS:** 0
**Total issues**: 399
Files checked: 1042
Total issues: 399

Δ from 2026-09-10 23:15 run (pending — prior run): +1 net change — total 398→399 (+1 individual broken), ERROR 0→0, WARNING 398→399, individual broken 379→380, forward-ref groups 19→19, unique targets 271→271. +4 files (2 concepts + 1 source + 3 topic pages, pipeline active after 9-day idle). Top-20 composition shifted: [[network-effects]] (3) entered, [[first-order-thinking]] (3) dropped to outside top 20. New debt from new source (src_0xhvdes-seven-ways-to-get-ahead.md) + concepts (proximity-to-opportunity, reputation-as-signal) — all forward-references only, no structural errors. KB growth +4, backlog essentially flat.

---

| Files checked | Concepts | Sources | Indexes | Topics |
|---|---|---|---|---|
| 1042 | 573 | 197 | 34 | 238 |

---

## Broken Wikilinks — Top 20 Targets

| Rank | Target | Count |
|---|---|---|
| 1 | [[game-theory]] | 10 |
| 2 | [[confirmation-bias]] | 8 |
| 3 | [[deep-work]] | 5 |
| 3 | [[ai-coding-agents]] | 5 |
| 3 | [[career-design]] | 5 |
| 3 | [[decision-making]] | 5 |
| 7 | [[attention-economy]] | 3 |
| 7 | [[ai-assisted-development]] | 3 |
| 7 | [[ai-hype-vs-reality]] | 3 |
| 7 | [[economic-inequality]] | 3 |
| 7 | [[intellectual-humility]] | 3 |
| 7 | [[network-effects]] | 3 |
| 7 | [[naval-ravikant]] | 3 |
| 7 | [[risk-parity]] | 3 |
| 7 | [[second-law-of-thermodynamics]] | 3 |
| 7 | [[homeostasis]] | 3 |
| 7 | [[saying-no]] | 3 |
| 7 | [[cognitive-dissonance]] | 3 |
| 7 | [[power-imbalance]] | 3 |
| 7 | [[stoicism]] | 3 |

## Forward-Reference Groups (19)

Source files with 4+ broken wikilinks to uncompiled concepts:

| File | Broken |
|---|---|
| wiki/sources/src_mental-models-of-art.md | 9 |
| wiki/sources/src_mental-models-of-economics.md | 9 |
| wiki/sources/src_fs-blog-mental-models.md | 7 |
| wiki/concepts/third-order-thinking.md | 6 |
| wiki/concepts/thought-experiment.md | 6 |
| wiki/sources/src_code-as-agent-harness-arxiv-2605-18747.md | 6 |
| wiki/sources/src_incentives-hidden-forces.md | 6 |
| wiki/sources/src_probabilistic-thinking.md | 6 |
| wiki/sources/src_11-minutes-hack-github.md | 4 |
| wiki/sources/src_ai-future-skills.md | 4 |
| wiki/sources/src_feedback-loops-mental-model.md | 4 |
| wiki/sources/src_farnam-street-mental-models-biology-series.md | 6 |
| wiki/sources/src_farnam-street-mental-models-systems-thinking.md | 6 |
| wiki/sources/src_global-macro-investing.md | 4 |
| wiki/sources/src_hermes-polymarket-btc-trading-agent.md | 4 |
| wiki/sources/src_the-cost-of-discretion.md | 4 |
| wiki/sources/src_the-seed-and-the-machine.md | 4 |
| wiki/sources/src_thought-experiment.md | 9 |
| wiki/sources/src_tribute-system-new-world-order.md | 4 |

## Verification

- [x] Read wiki/meta/format-spec.md (ground truth)
- [x] Scanned all wiki/sources/*.md + wiki/concepts/*.md
- [x] Validated frontmatter, sections, naming, markdown, wikilinks
- [x] Report generated (399 issues: 0E + 399W + 0I)
- [x] No file modifications — read-only validator

## Escalations

No escalations. Forward-reference backlog continues as the sole issue category. Pipeline active after 9-day idle — new files contributed 0 broken links to existing targets, +1 new individual forward-ref.

Standing note: forward-reference backlog (19 groups, 271 unique targets) persists across 4+ consecutive runs. This is a Compile Agent pipeline concern, not a format-spec issue.
