---
type: concept
status: draft
main_tag: productivity
sub_tags: [automation, tools]
topic: writing-content-system
sources:
  - "[[src_the-writing-habit-that-saved-my-brain]]"
last_updated: 2026-07-21
---

# Content Repurposing System

## Definition

Content repurposing system (hệ thống tái sử dụng nội dung) là quy trình biến một nội dung dài thành nhiều nội dung cho các nền tảng khác nhau. Dan Koe viết một newsletter/tuần và biến nó thành content cho tất cả các nền tảng.

## Key ideas

- Một chủ đề/tuần → newsletter dài form → X article → script YouTube → video → podcast
- Cả tuần xoay quanh chủ đề đó - khi đọc sách, xem video, ý tưởng phù hợp sẽ nổi bật
- Ý tưởng trở thành research cho chủ đề đang viết
- Sử dụng công cụ như Eden để lưu ý tưởng, bài viết, video, sách PDF và chat với chúng
- Không cần niche down - con ngườ có khả năng thích nghi, chỉ cần nói về điều mình thấy quan trọng
- Packaging ideas quan trọng như content itself - cần học cách viết hook, title thu hút
- Nghiên cứu những gì đang hoạt động trên social media để hiểu thị trường muốn gì

## Related concepts

- [[new-leverage-digital-assets]]
- [[forced-linearity-writing]]

## Sources

- [[src_the-writing-habit-that-saved-my-brain]]

## Notes

