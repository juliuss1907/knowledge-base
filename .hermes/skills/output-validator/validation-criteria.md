...
- Impossible date (e.g., 2026-13-45)
...
for match in re.finditer(date_pattern, text):
    ...
    if int(year) < 2000 or int(year) > 2030:
        → WARNING: Date outside reasonable range
...
All required sections present with sufficient detail and appropriate length.
...
**ERROR:**
- Missing required section (Definition, Summary, Key points, etc.)
- Section present but empty

**WARNING:**
- Optional section missing (Original excerpts, Notes)
- Section present but very short

**INFO:**
- Could add optional section
- Could expand existing section
...
**Optional sections:**
- `## Original excerpts` (sources)
- `## Notes` (concepts — Julius's annotations)
...
#### 2.3 Key Points Count

**ERROR:**
- Key points section missing
- Key points section empty
...
```python
key_points = extract_section(file, '## Key points')

# Count bullet points
points = re.findall(r'^- ', key_points, re.MULTILINE)
point_count = len(points)

if point_count == 0:
    → ERROR: No key points found

if point_count < 5:
    → WARNING: Too few key points ({point_count}, need 5-10)

if point_count > 10:
    → INFO: Too many key points ({point_count}, prefer 5-10)
```

#### 2.4 Definition Length (Concepts only)
...
**ERROR:**
- Sections in illogical order (e.g., conclusion before key ideas)
...
- Could use transitional phrase

**INFO:**
- Transition is acceptable but could be smoother
- Could add connecting sentence
...
Weak transitions:
- No transition (just new section header)
- Generic transition ("Next, ...")
