...
#
# Author: Julius
# Version: 1.0.1 (fixed submodule detection)
# =============================================================================
...
preflight() {
    ...
    # 1. Check we're in a git repo (handle both regular repo and submodule)
    if [[ ! -d ".git" ]] && [[ ! -f ".git" ]]; then
        abort "Not a git repository. Run from knowledge-base/ root."
    fi

    # If .git is a file (submodule), it contains gitdir pointer
    if [[ -f ".git" ]]; then
        log_ok "Git submodule detected"
    else
    ...
    if ! git show-ref --verify --quiet "refs/heads/$REQUIRED_BRANCH" 2>/dev/null; then
