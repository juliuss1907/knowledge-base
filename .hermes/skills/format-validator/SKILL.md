...
- **PENDING-prior-report baseline (observed 2026-09-02):** when the immediate prior report is still PENDING (Julius hasn't approved it yet, so Fix Agent took no action on it), that pending report is still the correct day-over-day baseline because it reflects the actual current KB state — no fixes were applied on top of it. Use it as the comparator and label the delta line `Δ from YYYY-MM-DD HH:MM run (pending — prior run)` so the status is explicit. The "APPROVED baseline" rule exists to avoid comparing against stale pre-fix numbers; a pending report with no applied fixes is not stale. Confirm no Fix Agent action landed on the pending report (git A-count since its timestamp) before using it as baseline. Note: this is the reverse of the 08-27 same-day case where an EARLIER same-day run WAS approved — when the prior run is approved use it; when it's pending use it anyway but say so.
...
### MEMORY.md sibling entry clobbered by fuzzy patch matching

When using `patch` on `MEMORY.md` (prepend-only, append-only file), the fuzzy matching can skip over sibling entries that were inserted between your read and write, replacing their content instead of the intended target. This is a distinct risk from the `_action-required.md` row-drop scenario: the fuzzy match finds the HEADER + TARGET pattern but jumps over intervening content, clobbering the sibling's heading.

**Observed 2026-09-02:** Output Validator (sibling subagent) prepended its `## 2026-09-02 23:09:11 — Output validation` entry to MEMORY.md between Format's read (23:17:31) and write (23:18:15). The patch's old_string spanned from the file header (`# Hermes — Memory Log\n\n> ...`) to `## 2026-09-01 23:15:37 — Format validation`. The fuzzy matcher skipped over the sibling's Output entry (which sat between the header and the Format anchor) and replaced the Output heading with Format content + a duplicate heading. The sibling's body text survived under the wrong heading, and the real 09-01 Format entry got pushed further down — silent data corruption that only a post-write read caught.

**Correct handling:**
1. **Re-read MEMORY.md** immediately before writing, same rule as `_action-required.md`. Do not rely on a cached read from earlier in the session.
2. **Use a targeted old_string that does NOT span the file header.** Anchor on the specific section heading you're inserting before (e.g. `old_string="## 2026-09-01 23:15:37 — Format validation"`), not a pattern from `# Hermes — Memory Log` down to that heading. The shorter anchor is far less likely to fuzzy-match over intervening sibling content.
3. **Verify MEMORY.md structure after writing** — read the first ~20 lines to confirm BOTH your entry AND the sibling's entry have correct headings. `ad-hoc-verify.py` checks sibling rows in `_action-required.md` but NOT the heading structure of MEMORY.md — add a manual grep for both `## YYYY-MM-DD` headings.
4. **If heading clobbering is detected**, fix with a single-line targeted patch restoring the sibling's correct heading (e.g. change the duplicate wrong heading back to `## 2026-09-02 23:09:11 — Output validation`).

The skill text already says "Same warning appears on `.hermes/MEMORY.md` patches (prepend-only, low risk)" — treat that as LOW RISK ONLY for the sibling-modification warning; the fuzzy-clobber scenario above is a real corruption that demands the post-write read.
...
**Observed 2026-09-01 — ad-hoc sibling check too strict:** the reusable script's sibling check (`if not siblings: errors.append("AR missing sibling PENDING row ...")`) hard-required a `🔍 PENDING` sibling row for the same date. It false-FAILED on a day when sibling validators legitimately had no PENDING row: Output ran `[SILENT]` (0 new files → no report written → no row) and Hygiene 09-01 was already `✅ APPLIED` earlier the same day. Fix (patched into the script 2026-09-01): iterate each sibling (Output/Hygiene) and accept its row in EITHER `🔍 PENDING` or `✅ APPLIED` form; only flag a missing row when that sibling's report file actually exists on disk (i.e. the row was genuinely dropped, not legitimately absent because the validator was SILENT or already applied). Verify the absence claim with `ls wiki/reviews/\<today\>_\<sibling\>-report.md` before trusting the tolerant path. This is the same emoji/state-divergence lesson as the earlier 'match on PENDING content, not a specific emoji' pitfall — now applied to the ad-hoc sibling check.
